...
def colormap_saving(image: torch.Tensor, colormap_options, save_path, original_image=None, alpha=0.7):
    """
    Create and optionally save a colormap visualization.
    
    Args:
        image: The heatmap/similarity map tensor to visualize
        colormap_options: Options for the colormap
        save_path: Where to save the result
        original_image: Optional original image to blend with the heatmap
        alpha: Blending coefficient (0.0 to 1.0) - how much of the heatmap to show
        
    Returns:
        The output image as a numpy array
    """
    # Generate colormap from the heatmap
    heatmap = colormaps.apply_colormap(
        image=image,
        colormap_options=colormap_options,
    ).cpu().numpy()
    
    # If no original image provided, just return the heatmap
    if original_image is None:
        output_image = heatmap
    else:
        # Make sure original image is in numpy format
        ...
        output_image = alpha * heatmap + (1 - alpha) * original_image

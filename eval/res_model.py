...
def load_model_hf(model_config_path, repo_id, filename, device='cpu'):
    ...
    cache_file = "/home/joanna_cheng/workspace/gaussian-grouping/Tracking-Anything-with-DEVA/saves/groundingdino_swint_ogc.pth"
...
sam_checkpoint_path = "/home/joanna_cheng/workspace/gaussian-grouping/Tracking-Anything-with-DEVA/saves/sam_vit_h_4b8939.pth" 
...
class RES_MODEL(nn.Module):
    ...
    def load_sam_model(self, device):
        ...
        sam_device = device
        self.sam_model = build_sam(checkpoint=sam_checkpoint_path).to(sam_device)
    ...
    def predict_res_mask(self, image, text_prompt):
        ...
        # Check if images_pil is valid
        if not images_pil or len(images_pil) == 0:
            print(f"Warning: No images generated for prompt '{text_prompt}'")
            # Return an empty mask with the same height and width as the input image
            empty_mask = torch.zeros((1, h, w), dtype=torch.bool)
